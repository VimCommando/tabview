...
- [x] 7.5 Add a source-neutral View Configuration modal that summarizes active view filters and sort precedence, clears those operations, toggles view-wide null placement, and opens current-column controls without re-querying the source.
- [x] 7.6 Keep Column Info as a contextual current-column editor with View filter and View sort controls backed by the same summarized `ViewTransform`, show source-operation summaries read-only, and keep all existing quick filter/sort commands view-scoped.
...
- [x] 8.8 Add render and interaction tests for selectable and disabled table-picker entries and their diagnostics; separate Source and View configuration modals; source draft/apply/cancel and capability states; View transform summaries and current-column navigation; view-only quick commands; shared Column Info/View filter and sort state; raw SQLite declarations and logical hints in Column Info; read-only source summaries; query progress; complete/limited extent; visible/source counts; SQL provenance; typed values; and errors.
...
- [x] 8.16 Run the all-feature test and Clippy matrix on the supported Linux host and record the result.
  - 2026-07-26 on `ironhide.local`: 386 all-feature unit tests and 16 non-interactive integration tests passed; all-target/all-feature Clippy passed with warnings denied. Five PTY integration cases could not launch because the host does not provide the external `script` utility.
