...
- Add separate Source and View configuration modals: Source edits adapter-dependent filters, sorting, limits, capabilities, and query provenance; View summarizes source-neutral local state and provides common actions without re-querying the source. Keep Column Info as the contextual current-column editor, and keep existing quick filter and sort commands view-scoped. A complete global View editor is deferred to `interface-updates`.
