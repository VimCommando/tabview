...
case "$target" in
  aarch64-apple-darwin) floor='macOS 14'; export MACOSX_DEPLOYMENT_TARGET=14.0 ;;
...
[ "$host" = "$target" ] || { echo 'Packaging requires native execution on the target' >&2; exit 1; }
if [ "$#" -ge 3 ]; then
  binary=$3
else
